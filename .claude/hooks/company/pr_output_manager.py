...
def enable_github_auto_merge(pr_url_or_number: str, *, method: str = "squash") -> bool:
    ...
    3. Refuses if the base branch has no required status checks OR the repo has no
    ...
    4. Returns False on any gh fetch or merge error (fail closed).
    ...
    # Guard 3: refuse on gateless repos (no required checks + no CI history).
...
def run_deliverable_gate(
    ...
    if not verdict.blocked and automerge_to_main_allowed():
        auto_merge_enabled = enable_github_auto_merge(pr_number)
