...
def _should_run_idea_auto_convert(config: DaemonConfig, state: DaemonState) -> bool:
    """Time-gate the approved-idea auto-conversion.

    Piggybacked on the ideation cycle: runs once per ``employee_ideation_interval_minutes``.
...
def _check_security_gates(
    ...
    Gates:
        ...
        4. Diff size — must be within configured thresholds
...
def _get_launchagent_path() -> Path | None:
    ...
    try:
        ...
        plist_path = (
            ...
            / "Library"
            / "LaunchAgents"
            / f"com.forgelabs.daemon.{project_id}.plist"
        )
